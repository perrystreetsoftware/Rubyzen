...
RSpec.describe 'be_false matcher' do
  let(:file) do
    parse_ruby(<<~RUBY)
      class Foo
        def bar; end
  ...
  it 'passes when block returns false for all elements' do
    expect(methods).to be_false { |m| m.parameters? }
  end
  ...
  it 'supports custom failure message' do
    ...
    methods = file.classes.first.instance_methods
    expect {
      expect(methods).to be_false("No methods should have params") { |m| m.parameters? }
  ...
  describe 'with allowlist' do
    it 'passes when failing items are allowlisted' do
      ...
      methods = file.classes.first.instance_methods
      expect(methods).to be_false(allowlist: ['bar']) { |m| m.parameters? }
    end
    ...
    it 'fails on stale allowlist entries' do
      ...
      methods = file.classes.first.instance_methods
      expect {
        expect(methods).to be_false(allowlist: ['nonexistent']) { |m| m.parameters? }
  ...
  describe 'with baseline' do
    it 'passes when failing items are in baseline' do
      ...
      methods = file.classes.first.instance_methods
      expect(methods).to be_false(baseline: ['bar']) { |m| m.parameters? }
    end
    ...
    it 'fails on stale baseline entries' do
      ...
      methods = file.classes.first.instance_methods
      expect {
        expect(methods).to be_false(baseline: ['ghost']) { |m| m.parameters? }
      }.to raise_error(RSpec::Expectations::ExpectationNotMetError, /stale/i)
    end
  end
end
