...
RSpec::Matchers.define :have_top_level_module do |expected_module, custom_message=nil|
  ...
  match do |classes_collection|
    ...
    @offenders = classes_collection.select do |class_decl|
      class_decl.top_level_module != @expected_module
    end.map(&:name)
  ...
  failure_message do |_classes_collection|
    message_for_failure(
      "Expected all classes to be defined under '#{@expected_module}' module, but these were not: #{@offenders.join(', ')}"
    )
  end

  failure_message_when_negated do |_classes_collection|
    message_for_failure(
      "Expected some classes to be defined under '#{@expected_module}' module, but none were."
    )
  end
end
