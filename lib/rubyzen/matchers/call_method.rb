...
RSpec::Matchers.define :call_method do |method_names, on_receivers: nil, message: nil|
  ...
  match do |classes_collection|
    ...
    @offenders = classes_collection.select do |class_info|
      if @on_receivers
        ...
        (@method_names & class_info.called_method_names).any?
      end
    end.map(&:name)
  ...
  failure_message do |classes_collection|
    ...
    message_for_failure(@custom_message || "Expected some classes to call `#{methods_list}`#{receiver_msg}, but none did.")
  end

  failure_message_when_negated do |classes_collection|
    methods_list = @method_names.size > 1 ? @method_names.join(', ') : @method_names.first
    receiver_msg = @on_receivers ? " on #{@on_receivers.join(', ')}" : ""
    message_for_failure(@custom_message || "Expected no classes to call `#{methods_list}`#{receiver_msg}, but these classes did: #{@offenders.join(', ')}")
  end
end
